...
export default function Industries() {
  ...
  useEffect(() => {
    // Only scroll the sidebar list into view AFTER the user has interacted —
    // never on initial mount (otherwise it forces the whole page to jump here).
    if (!userInteractedRef.current) return;
    const list = sidebarRef.current;
    const el = list?.querySelector(`[data-idx="${active}"]`);
    if (!el || !list) return;

    // Scroll only the sidebar UL, not the whole page.
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1440px] px-6 md:px-10 lg:px-16">
        ...
        <div className="mt-14 hidden md:grid grid-cols-12 gap-px bg-[#2B313A]/60 border border-[#2B313A] min-h-[560px]">
          {/* Sidebar */}
          <aside
            ref={sidebarRef}
            className="col-span-4 lg:col-span-3 bg-[#0D1117] overflow-y-auto max-h-[560px] industrial-scroll"
          >
            <div className="px-4 py-3 border-b border-[#2B313A] flex items-center justify-between">
              <span className="font-[Orbitron] text-[10px] tracking-[0.3em] text-[#E6EDF3]/40">VERTICALS</span>
              <span className="font-[Orbitron] text-[10px] tracking-[0.25em] text-[#00C2FF] tabular">{String(VERTICALS.length).padStart(2, "0")}</span>
            </div>
            <ul>
                ...
                return (
                  <li key={vert.id} data-idx={i}>
                    <button
                      ...
                      onMouseEnter={() => setActiveByUser(i)}
                      onFocus={() => setActiveByUser(i)}
                      onClick={() => setActiveByUser(i)}
                      className="w-full text-left px-4 py-3 flex items-center gap-3 border-b border-[#2B313A] group transition-colors"
                      style={{
                        background: isOn ? "rgba(31,143,255,0.06)" : "transparent",
                        color: isOn ? "#E6EDF3" : "rgba(230,237,243,0.7)",
                      }}
                    >
                      <span className="w-1 h-6 transition-colors" style={{ background: isOn ? "#00C2FF" : "#2B313A" }} />
                      <span className="font-[Orbitron] text-[9px] tracking-[0.25em] w-12 shrink-0" style={{ color: isOn ? "#00C2FF" : "#2B313A" }}>
                        {vert.code.replace("//05.", "")}
                      </span>
          ...
          {/* Detail */}
          <div className="col-span-8 lg:col-span-9 bg-[#0D1117] relative overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="col-span-7 flex flex-col">
                  <div className="font-[Orbitron] text-[10px] tracking-[0.3em] text-[#00C2FF]">{v.code}</div>
                  <h3 className="font-[Montserrat] text-2xl lg:text-3xl font-medium text-[#E6EDF3] mt-3 leading-tight">
                    {v.name}
                  </h3>
                  <p className="mt-5 text-[#E6EDF3]/65 text-sm lg:text-base font-light leading-relaxed">
                    {v.body}
                  </p>
                  <ul className="mt-7 space-y-1.5">
                    {v.items.map((it) => (
                      <li key={it} className="flex items-center gap-3 text-[#E6EDF3]/75 text-sm">
                        <span className="w-2 h-px bg-[#00C2FF]" />
                ...
                <div className="col-span-5 relative">
                  ...
                  <div className="relative w-full h-full min-h-[280px] border border-[#2B313A] overflow-hidden">
                    <motion.img
                      key={v.photo}
                      src={v.photo}
                      alt={v.name}
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      <div className="absolute top-2 left-2 right-2 flex items-center justify-between font-[Orbitron] text-[9px] tracking-[0.3em] text-[#E6EDF3]/70">
                        <span>VERTICAL · {v.code}</span>
                        <span className="text-[#00C2FF]">● LIVE</span>
                      </div>
                      <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between font-[Orbitron] text-[9px] tracking-[0.3em] text-[#E6EDF3]/70">
                        <span className="truncate uppercase">{v.name}</span>
                        <span className="text-[#00C2FF]">REF · {String(active + 1).padStart(2, "0")}/{VERTICALS.length}</span>
                      </div>
                      {/* Corner ticks */}
        ...
        <div className="mt-10 md:hidden space-y-px bg-[#2B313A]/60 border border-[#2B313A]">
          {VERTICALS.map((vert) => (
            <details key={vert.id} className="bg-[#0D1117]" data-testid={`industry-mobile-${vert.id}`}>
              <summary className="cursor-pointer list-none p-5 flex items-center justify-between">
                <div>
                  <div className="font-[Orbitron] text-[9px] tracking-[0.3em] text-[#E6EDF3]/40">{vert.code}</div>
                  <div className="font-[Montserrat] text-base mt-1 text-[#E6EDF3]">{vert.name}</div>
                </div>
                <span className="font-[Orbitron] text-[#00C2FF] text-xs">[ + ]</span>
              </summary>
              <div className="p-5 pt-0 space-y-4">
                <div className="relative aspect-video border border-[#2B313A] overflow-hidden">
                  <img src={vert.photo} alt={vert.name} className="absolute inset-0 w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-[#0D1117]/45 mix-blend-multiply" />
                </div>
                <p className="text-[#E6EDF3]/60 text-sm font-light leading-relaxed">{vert.body}</p>
                <ul className="space-y-1.5">
                  {vert.items.map((it) => (
                    <li key={it} className="flex items-center gap-2 text-[#E6EDF3]/70 text-xs">
                      <span className="w-2 h-px bg-[#00C2FF]" />
                      <span>{it}</span>
                    </li>
                  ))}
