    ...
    >
      <div className="mx-auto max-w-[1440px] px-6 md:px-10 lg:px-16">
        ...
        <div className="mt-20 relative">
          {/* Central line */}
          <div className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-px bg-tasc-border" />
          <div
            className="absolute left-1/2 top-0 -translate-x-1/2 w-px bg-tasc-cyan"
            style={{
              height: `${progress * 100}%`,
...
function NodeMarker({ code, lit }) {
  return (
    <div className="order-2 col-span-0 flex items-center justify-center relative">
      <div className="absolute left-1/2 -translate-x-1/2 w-12 h-12 border border-tasc-border bg-tasc-bg flex items-center justify-center transition-all duration-500"
        style={{
          borderColor: lit ? 'var(--tasc-cyan)' : 'var(--tasc-border)',
          boxShadow: lit ? "0 0 24px -4px rgba(31,143,255,0.6)" : "none",
        }}
      >
        <span
          className="font-[Orbitron] text-[10px] tracking-[0.2em] transition-colors"
          style={{ color: lit ? 'var(--tasc-cyan)' : 'var(--tasc-text)' }}
        >
          {code}
        </span>
      </div>
...
function NodeContent({ node, lit, align }) {
  return (
    <div className={align === "right" ? "ml-auto max-w-md" : "mr-auto max-w-md"}>
      <h3 className="font-[Montserrat] text-xl md:text-2xl font-medium text-tasc-text leading-tight">
        {node.title}
      </h3>
      <p className="mt-3 text-tasc-text/55 text-sm leading-relaxed font-light">
        {node.body}
      </p>
    </div>
  );
