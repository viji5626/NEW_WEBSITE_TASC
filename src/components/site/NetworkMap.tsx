    ...
    >
      <defs>
        ...
        <pattern id="iso" x="0" y="0" width="40" height="23" patternUnits="userSpaceOnUse">
          <path d="M0 11.5 L40 11.5 M20 0 L20 23" stroke='var(--tasc-border)' strokeWidth="0.5" opacity="0.35" />
        </pattern>
      ...
      {particles.map((p) => (
        <circle key={p.id} r="1" fill='var(--tasc-cyan)' opacity="0.35">
          <animate
            attributeName="cy"
            from={p.cy - 20}
            to={p.cy + 20}
      ...
      {/* Outer rings */}
      <circle cx="400" cy="300" r="240" stroke='var(--tasc-border)' strokeWidth="1" fill="none" strokeDasharray="2 4" />
      <circle cx="400" cy="300" r="160" stroke='var(--tasc-border)' strokeWidth="1" fill="none" strokeDasharray="2 4" />
      <circle cx="400" cy="300" r="240" stroke='var(--tasc-cyan)' strokeWidth="0.5" fill="none" opacity="0.25">
      ...
      {PATHS.map((p, i) => (
        <g key={p.id + "-vis"}>
          <path d={p.d} stroke='var(--tasc-border)' strokeWidth="1" fill="none" />
          <path
            d={p.d}
            stroke='var(--tasc-cyan)'
            strokeWidth="1.25"
            fill="none"
            className="flow-path"
      ...
      {PATHS.map((p, i) => (
        <g key={p.id + "-pkt"}>
          <circle r="2.5" fill='var(--tasc-cyan)' filter="url(#glow)">
            <animateMotion dur={`${p.dur}s`} repeatCount="indefinite" begin={`${(i * 0.3) % 2}s`}>
              <mpath href={`#${p.id}`} />
            </animateMotion>
          </circle>
          {/* Occasional larger burst packet */}
          <circle r="4" fill='var(--tasc-cyan)' filter="url(#strongGlow)" opacity="0.9">
      ...
      {NODES.map((n, i) => {
        ...
        return (
          <g key={n.label}>
            ...
            {/* Burst ring */}
            {isBursting && (
              <circle cx={n.x} cy={n.y} r="6" fill="none" stroke='var(--tasc-cyan)' strokeWidth="1.2" opacity="0.9">
                <animate attributeName="r" from="6" to="26" dur="0.6s" />
                <animate attributeName="opacity" from="0.9" to="0" dur="0.6s" />
              </circle>
            )}
            <circle cx={n.x} cy={n.y} r={isBursting ? "3" : "2"} fill='var(--tasc-cyan)' filter="url(#glow)" />
            <text
              x={n.x}
              y={n.y + 28}
              fontSize="8"
      ...
      {/* Central TASC node */}
      <g>
        <rect x="350" y="280" width="100" height="40" stroke='var(--tasc-cyan)' strokeWidth="1.25" fill='var(--tasc-bg)' />
        <rect x="346" y="276" width="108" height="48" stroke='var(--tasc-cyan)' strokeWidth="0.5" fill="none" opacity="0.4">
          <animate attributeName="opacity" values="0.2;0.6;0.2" dur="3s" repeatCount="indefinite" />
        </rect>
        <text x="400" y="298" textAnchor="middle" fontSize="10" fill='var(--tasc-cyan)' fontFamily="Orbitron, sans-serif" letterSpacing="3">
          TASC
        </text>
        <text x="400" y="312" textAnchor="middle" fontSize="6" fill='var(--tasc-text)' opacity="0.6" fontFamily="Orbitron, sans-serif" letterSpacing="2">
          CORE //
        </text>
      </g>
